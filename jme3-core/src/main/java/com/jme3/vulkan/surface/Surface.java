...
public class Surface extends AbstractNative<Long> implements DeviceFilter {
    ...
    public Float evaluateDevice(PhysicalDevice device) {
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            KHRSurface.vkGetPhysicalDeviceSurfaceFormatsKHR(device.getHandle(), object, count, null);
            if (count.get(0) == 0) {
                System.out.println("Reject device by surface support (formats)");
                return null;
            }
            KHRSurface.vkGetPhysicalDeviceSurfacePresentModesKHR(device.getHandle(), object, count, null);
            if (count.get(0) == 0) {
