...
public class LoadedMaterial implements AssetLoader {
    ...
    public Object load(AssetInfo assetInfo) throws IOException {
        ...
        try {
            mapper.readValue(stream, LoadedMaterial.class);
            JsonNode root = mapper.readTree(stream);
            JsonNode imports = root.get("imports");
            JsonNode parameters = root.get("parameters");
            parameters.fields();
            ReflectionArgs args = new ReflectionArgs(assetInfo.getManager(), generator, imports);
            for (Iterator<Map.Entry<String, JsonNode>> it = parameters.fields(); it.hasNext();) {
                ...
                sets.computeIfAbsent(set, n -> new ArrayList<>()).add(
                        args.create(param.getKey(), param.getValue()).instantiate());
            }
            ...
            for (Iterator<Map.Entry<String, JsonNode>> it = pipelines.fields(); it.hasNext();) {
                ...
                PipelineState<?> state = args.create(pipeline.getKey(), "GraphicsState", pipeline.getValue()).instantiate();
            }
