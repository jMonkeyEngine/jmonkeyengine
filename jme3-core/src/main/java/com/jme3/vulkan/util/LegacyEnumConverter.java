...
public class LegacyEnumConverter {
    ...
    public static int imageAspects(Format.Aspects aspects) {
        int bits = 0;
        if (aspects.isColor()) bits |= VK10.VK_IMAGE_ASPECT_COLOR_BIT;
        if (aspects.isDepth()) bits |= VK10.VK_IMAGE_ASPECT_DEPTH_BIT;
        if (aspects.isStencil()) bits |= VK10.VK_IMAGE_ASPECT_STENCIL_BIT;
        return bits;
    }
