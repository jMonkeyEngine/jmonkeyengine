...
import static com.jme3.renderer.framegraph.FrameGraph.RENDER_THREAD;
 ...
 * @author codex
 */
public class PassQueueExecutor implements Runnable, Iterable<RenderPass> {
    ...
    public void run() {
        ...
        for (RenderPass p : queue) {
            if (p.isUsed()) {
                /*if (!async) {
                    if (context.isProfilerAvailable()) context.getProfiler().fgStep(FgStep.Execute, p.getProfilerName());
