 ...
 */
package com.jme3.input.vr.lwjgl;
 ...
 */
public class OpenVRInputLwjgl implements VRInputAPI {

    private static final Logger logger = Logger.getLogger(OpenVRInputLwjgl.class.getName());
     ...
     */
    public OpenVRInputLwjgl(VREnvironment environment) {
    ...
    public boolean isButtonDown(int controllerIndex, VRInputType checkButton) {
        VRControllerState cs = cStates[OpenVRInputLwjgl.controllerIndex[controllerIndex]];
        switch (checkButton) {
            default:
                return false;
            case ViveGripButton:
    ...
    public boolean wasButtonPressedSinceLastCall(int controllerIndex, VRInputType checkButton) {
        ...
        int cIndex = OpenVRInputLwjgl.controllerIndex[controllerIndex];
    ...
    public Vector3f getVelocity(int controllerIndex) {
        ...
        if (environment != null) {
            ...
            if (environment.getVRHardware() instanceof OpenVrLwjgl) {
                int index = OpenVRInputLwjgl.controllerIndex[controllerIndex];
//                if( needsNewVelocity[index] ) {
                HmdVector3 tempVec = ((OpenVrLwjgl) environment.getVRHardware()).hmdTrackedDevicePoses[index].vVelocity();
...
//                }
                ...
                throw new IllegalStateException("VR hardware " + environment.getVRHardware().getClass().getSimpleName() + " is not a subclass of " + OpenVrLwjgl.class.getSimpleName());
            }
        } else {
    ...
    public Vector3f getAngularVelocity(int controllerIndex) {
        ...
        if (environment != null) {
            ...
            if (environment.getVRHardware() instanceof OpenVrLwjgl) {
                ...
                int index = OpenVRInputLwjgl.controllerIndex[controllerIndex];
                HmdVector3 tempVec = ((OpenVrLwjgl) environment.getVRHardware()).hmdTrackedDevicePoses[index].vAngularVelocity();
//                    needsNewVelocity[index] = false;
//                }
                tempVel.x = tempVec.v(0);
                tempVel.y = tempVec.v(1);
...
//    	}      
    ...
    public boolean isInputDeviceTracking(int index) {
        ...
        if (environment != null) {
            ...
            if (environment.getVRHardware() instanceof OpenVrLwjgl) {
                return ((OpenVrLwjgl) environment.getVRHardware()).hmdTrackedDevicePoses[controllerIndex[index]].bPoseIsValid();
            } else {
                throw new IllegalStateException("VR hardware " + environment.getVRHardware().getClass().getSimpleName() + " is not a subclass of " + OpenVrLwjgl.class.getSimpleName());
            }
        } else {
    ...
    public Quaternion getOrientation(int index) {
        ...
        if (environment != null) {
            ...
            if (environment.getVRHardware() instanceof OpenVrLwjgl) {
                ...
                VRUtil.convertMatrix4toQuat(((OpenVrLwjgl) environment.getVRHardware()).poseMatrices[index], rotStore[index]);
    ...
    public Vector3f getPosition(int index) {
        ...
        if (environment != null) {
            ...
            if (environment.getVRHardware() instanceof OpenVrLwjgl) {
                ...
                index = controllerIndex[index];
                ((OpenVrLwjgl) environment.getVRHardware()).poseMatrices[index].toTranslationVector(posStore[index]);
    ...
    public void triggerHapticPulse(int controllerIndex, float seconds) {
        ...
        VRSystem.VRSystem_TriggerHapticPulse(OpenVRInputLwjgl.controllerIndex[controllerIndex],
